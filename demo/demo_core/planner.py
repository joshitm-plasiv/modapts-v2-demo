...
def _system(line_names: list[str]) -> str:
    return (
        ...
        "assistant. You see the CONVERSATION SO FAR and the user's LATEST message, and "
        ...
        "  explain      — answer ANY open or analytical question with reasoning, no "
        "re-measuring: the reasoning behind a result (\"why is that M4?\", \"are you sure?\", "
        "\"how did you get that?\", \"break it down\"), comparisons (\"compare MODAPTS vs MOST "
        "here\", \"why does this line miss target?\"), plausibility (\"is 40.5 s realistic?\"), "
        "the methodology/conventions (\"what does the banding mean?\"), or trade-off / "
        "what-would-change discussion. args: none. Use this for anything that is NOT a new "
        "measurement, a line analysis, a parametric sweep, a correction, or a code edit.\n"
        ...
        "that ...\".\n"
        "  code_edit    — the user dictates an exact MODAPTS code for an operation. It is "
        "priced from the standard table (shows MODs and time), validated token-by-token, and "
        "recorded as a teaching example. args: text (the operation), code (the code string). "
        "Use for \"set the code to ...\", \"the code should be ...\".\n"
